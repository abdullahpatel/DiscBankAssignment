...
class DiscBankAssignmentApplicationTests {
	...
	public void testUseCase1_service() {
		...
		List<ClientAccount> accounts = clientAccService.findTransactionalAccountByClient(new Long(1),Calendar.getInstance());
		log.info("accounts.size()="+accounts.size());
		for (ClientAccount account : accounts) {
	...
	public void testUseCase2_service() {
		...
		List<CurrencyAccount> accounts = clientAccService.findCurrencyAccountsByClient(new Long(2), Calendar.getInstance());
		log.info("accounts.size()="+accounts.size());
	...
	public void testUseCase3_withdrawCash() {
		...
		clientAccService.withdrawCash(atmId, clientId, accountNumber, requiredAmount, Calendar.getInstance());
	}
